...
def read_data_list(args, data_dir, pattern, txtfile):
    ...
    print(
        f'The size of  \"{args.patch_dir}/{pattern}\" = {len(data_list)}, equal ply = {int(len(data_list) / args.patch_num)}')
    return data_list


def load_data(message, args, data_dir, pattern):
...
class WPC_SD(Dataset):
    def __init__(self, args, data_dir, pattern, txtfile):
        self.num_points = args.point_num
        self.data_dir = data_dir
        self.pattern = pattern
        self.data_list = read_data_list(args, data_dir, pattern, txtfile)
        self.data_len = len(self.data_list)
        self.args = args

    def __getitem__(self, item):
        message = self.data_list[item]
        point, mos, filenum = load_data(message, self.args, self.data_dir, self.pattern)
