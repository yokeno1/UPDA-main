...
def read_data_list(args, csvfile):
    ...
    for i, row in df.iterrows():
        filename_without_ext = os.path.splitext(row['name'])[0]  # 去掉.ply后缀
        value = row['mos']  # 获取数值

        # 根据patch_num生成多个数据项
        for patch_idx in range(args.patch_num):
            shape_id = filename_without_ext
            npy_filename = f"{filename_without_ext}__{patch_idx}.npy"
            # 保持与原始格式相同的结构
            data_item = [shape_id, npy_filename, str(value), i, str(patch_idx)]
            data_list.append(data_item)

    print(
        f'The size of  \"{args.patch_dir}\" = {len(data_list)}, equal ply = {int(len(data_list) / args.patch_num)}')
    return data_list
    
def load_data(message, args, data_dir):
    npy_dir = f'{data_dir}/{args.patch_dir}/{message[0]}/{message[1]}'
...
class WPC_SD(Dataset):
    def __init__(self, args, data_dir, filename_list):
        self.num_points = args.point_num
        self.data_dir = data_dir
        self.data_list = read_data_list(args, filename_list)
        self.data_len = len(self.data_list)
        self.args = args
        
    def __getitem__(self, item):
        message = self.data_list[item]
        point, mos, filenum= load_data(message, self.args, self.data_dir)
