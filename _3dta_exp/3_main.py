...
def copy_code(results_dir):
    ...
    shutil.copy('../dataload/data_load.py', f'{results_dir}/data_load.py')
...
def train(args, i_fold):
    ...
    elif args.database == 'WPC2.0':
        train_filename_list = 'datainfo/wpc2.0_data_info/train_' + str(i_fold) + '.csv'
        test_filename_list = 'datainfo/wpc2.0_data_info/test_' + str(i_fold) + '.csv'

    train_data = WPC_SD(args, train_filename_list)
    train_loader = DataLoader(train_data, num_workers=4,
                            batch_size=args.batch_size, shuffle=True, drop_last=True)
    test_data = WPC_SD(args, test_filename_list)
    ...
    model_path = f'{args.database}_model.pth'
    ...
    for epoch in range(args.epochs):
        ...
        time_now = f'{time.strftime("%Y/%m/%d %H:%M:%S", time.localtime())}'
        with open(args.results_dir + f'/{args.database}_train_log.txt', 'a+') as txt:
            txt.write(f'\n{time_now}    {record}')
        ...
        if epoch==99:                               # record score epoch=100
...
if __name__ == "__main__":
    ...
    parser.add_argument('--results_dir', type=str, default='./checkpoints/baseline', metavar='N', help='Where does dataset exist?')
    ...
    parser.add_argument('--database', type=str, default='SJTU', metavar='N')
    parser.add_argument('--data_dir', type=str, default='/mnt/d/xbx/data/sjtu', metavar='N', help='Where does dataset exist?')
    # parser.add_argument('--database', type=str, default='WPC', metavar='N')
    # parser.add_argument('--data_dir', type=str, default='/mnt/d/xbx/data/wpc', metavar='N', help='Where does dataset exist?')
    # parser.add_argument('--database', type=str, default='WPC', metavar='N')
    # parser.add_argument('--data_dir', type=str, default='/mnt/d/xbx/data/wpc', metavar='N', help='Where does dataset exist?')
