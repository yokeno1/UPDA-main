...
class Pct_3DTA(nn.Module):
    def __init__(self, args, final_channels=1):
        ...
        self.args = args
        self.feature_ex = Feature_extraction(args)

        self.rank_ad_net = rank_net(in_feature=args.in_size)

        self.mapping = fusion_net(in_feature=args.in_size)
        self.ad_net = AdversarialNetwork(in_feature=args.in_size)
    ...
    def forward(self, x, x2, mos, if_train):
        if if_train:
            batch_size, _, _ = x.size()
            feat = self.feature_ex(x)
            feat2 = self.feature_ex(x2)

            feat = F.adaptive_max_pool1d(feat, 1).view(batch_size, -1)
            feat2 = F.adaptive_max_pool1d(feat2, 1).view(batch_size, -1)

            rank_loss = self.rank_ad_net(feat, feat2, mos)

            feat_map, t_feat_map = self.mapping(feat, feat2, if_train)
            ...
            x = self.dp2(x)
            score1 = self.linear3(x)

            x = F.leaky_relu(self.bn6(self.linear1(feat_map)), negative_slope=0.2)
            x = self.dp1(x)
            x = F.leaky_relu(self.bn7(self.linear2(x)), negative_slope=0.2)
            x = self.dp2(x)
            score = self.linear3(x)

            return domain_out, t_domain_out, score1, score, rank_loss[0], rank_loss[1]
        else:
            batch_size, _, _ = x.size()
            feat = self.feature_ex(x)
            feat2 = self.feature_ex(x2)
